...
type Draft = {
  section_a: { hospital: "" | "nkanyezi" | "fountain" | "mediclinic_vereeniging" | "midvaal"; title: string; first_names: string; surname: string; id_type: "sa_id" | "passport" | "other"; id_number: string; id_country?: string; email: string; phone: string; address: string };
...
const emptyDraft: Draft = {
  section_a: { hospital: "", title: "Mr", first_names: "", surname: "", id_type: "sa_id", id_number: "", email: "", phone: "+27", address: "" },
        ...
        {step === 0 && (
          <>
            ...
            <div className="grid grid-cols-2 gap-4">
              <Field label="Hospital" required full>
                <select
                  className="input"
                  value={A.hospital}
                  onChange={(e) => setDraft({ ...draft, section_a: { ...A, hospital: e.target.value as Draft["section_a"]["hospital"] } })}
                >
                  <option value="" disabled>Select hospital…</option>
                  <option value="nkanyezi">Nkanyezi Private Hospital</option>
                  <option value="fountain">Fountain Private Hospital</option>
                  <option value="mediclinic_vereeniging">Mediclinic Vereeniging Hospital</option>
                  <option value="midvaal">Midvaal Private Hospital</option>
                </select>
              </Field>
              <Field label="Title">
        ...
        {step < STEPS.length - 1 && (
          <div className="flex justify-between pt-4 border-t border-border-subtle">
            ...
            <button
              ...
              disabled={step === 0 && !A.hospital}
              title={step === 0 && !A.hospital ? "Select a hospital to continue" : undefined}
