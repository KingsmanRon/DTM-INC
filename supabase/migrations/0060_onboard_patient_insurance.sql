-- 0060_onboard_patient_insurance.sql — onboard_patient v5.
--
-- Single change from the 0057 body: the patient_medical_aid insert now also
-- writes group_number and subscriber_relationship (0059). This is locale-
-- AGNOSTIC — both are nullif(...)'d from section C, so they are NULL for SA
-- (whose payload has no such keys) and populated for US insurance. No new locale
-- branch. Everything else is byte-identical to 0057.
--
...
begin
  ...
  insert into public.patient_medical_aid (
    ...
    nullif(p_section_c->>'group_number', ''),
    nullif(p_section_c->>'subscriber_relationship', ''),
    v_actor_user_id,
    v_actor_user_id
  );
...
-- privileges; re-asserted here to match 0017/0045/0057).
