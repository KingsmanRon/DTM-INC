-- Add mandatory hospital field to patients and use its abbreviation as the
-- file-number prefix for newly onboarded patients.
--
-- Hospitals and their file-number prefixes:
--   nkanyezi               -> NKA   (Nkanyezi Private Hospital)
--   fountain               -> FOU   (Fountain Private Hospital)
--   mediclinic_vereeniging -> MED   (Mediclinic Vereeniging Hospital)
--   midvaal                -> MID   (Midvaal Private Hospital)

create type hospital_enum as enum (
  'nkanyezi',
  'fountain',
  'mediclinic_vereeniging',
  'midvaal'
);

alter table patients
  add column if not exists hospital hospital_enum;

-- Backfill existing rows: the practice's primary site is Nkanyezi.
update patients set hospital = 'nkanyezi' where hospital is null;

alter table patients
  alter column hospital set not null;

create index if not exists patients_hospital_idx on patients (hospital);

-- Replace onboard_patient to (a) accept hospital from section_a, (b) derive
-- the file-number prefix from it, (c) persist hospital on the patients row.
...
declare
  ...
  v_dep         jsonb;
  v_payer       payer_type;
  v_hospital    hospital_enum;
  v_prefix      text;
begin
  v_hospital := (p_section_a->>'hospital')::hospital_enum;

  v_prefix := case v_hospital
    when 'nkanyezi'               then 'NKA'
    when 'fountain'               then 'FOU'
    when 'mediclinic_vereeniging' then 'MED'
    when 'midvaal'                then 'MID'
  end;
