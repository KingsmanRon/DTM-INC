-- Add minor flag + ID guardrails for onboarding.

alter type id_type add value if not exists 'none_minor';

alter table patients
  add column if not exists is_minor boolean;

update patients
set is_minor = coalesce(is_minor, false)
...
alter table patients
  ...
  check (
    (not is_minor and id_type in ('sa_id', 'passport') and btrim(id_number) <> '' and (id_type <> 'passport' or id_country is not null))
    or
    (is_minor and ((id_type = 'none_minor') or (id_type in ('sa_id', 'passport') and btrim(id_number) <> '' and (id_type <> 'passport' or id_country is not null))))
  );
...
begin
  ...
  v_is_minor := coalesce((p_section_a->>'is_minor')::boolean, false);
  ...
  if not v_is_minor and (p_section_a->>'id_type') = 'none_minor' then
    raise exception 'Adults must use SA ID or passport';
  end if;

  if v_is_minor and (
    nullif(p_section_b->>'first_names', '') is null
    or nullif(p_section_b->>'surname', '') is null
    or nullif(p_section_b->>'id_number', '') is null
  ) then
  ...
  insert into patients (
    file_number, hospital, is_minor, title, first_names, surname, id_number, id_type, id_country,
